    ...
    void generateSystemCode() {
        FastAutoGenerator.create("jdbc:mysql://localhost:3308/vhr02?serverTimezone=Asia/Shanghai", "root", "root")
                .globalConfig(builder -> {
                    builder.author("waterkid") // 设置作者
//                            .enableSwagger() // 开启 swagger 模式
//                            .fileOverride() // 覆盖已生成文件D:\waterkid\vhr\vhr\vhr-system\src\main\java\com\cj\system
                            .outputDir("D:\\waterkid\\vhr\\vhr\\vhr-system\\src\\main\\java"); // 指定输出目录
                })
                .dataSourceConfig(builder -> builder.typeConvertHandler((globalConfig, typeRegistry, metaInfo) -> {
                    int typeCode = metaInfo.getJdbcType().TYPE_CODE;
                    if (typeCode == Types.SMALLINT) {
                ...
                .packageConfig(builder -> {
                    builder.parent("com.cj") // 设置父包名
                            ...
                            .pathInfo(Collections.singletonMap(OutputFile.xml, "D:\\waterkid\\vhr\\vhr\\vhr-system\\src\\main\\resources\\mapper")); // 设置mapperXml生成路径
                })
                .strategyConfig(builder -> {
